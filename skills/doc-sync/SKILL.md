...
Maintains the GEMINI.md navigation hierarchy and README.md invisible knowledge
docs across a repository. This skill is self-contained and performs all
documentation work directly.

## Documentation Conventions

For authoritative GEMINI.md and README.md format specification:

<file working-dir=".gemini" uri="conventions/documentation.md" />
...
| "update GEMINI.md for parser.py"                        | FILE: single file's parent directory      |
...
Map directories requiring GEMINI.md verification:
...
**Critical:** If GEMINI.md contains content that does NOT belong there, migrate it:

Content that MUST be moved from GEMINI.md to README.md:
...
Content that MAY stay in GEMINI.md (operational sections):
...
(commands, procedures) stays in GEMINI.md.

Migration process:

1. Identify misplaced content in GEMINI.md
2. Create or update README.md with the architectural content
3. Strip GEMINI.md down to pure index format
4. Add README.md to the GEMINI.md index table

### Phase 4: Index Updates

For each directory needing work:

**Creating/Updating GEMINI.md:**
...
1. Every directory in scope has GEMINI.md
2. All GEMINI.md files use table-based index format (pure navigation)
3. No drift remains (files <-> index entries match)
4. No misplaced content in GEMINI.md (explanatory prose moved to README.md)
5. README.md files are indexed in their parent GEMINI.md
6. GEMINI.md contains only: one-sentence overview + tabular index + operational sections
...
### Changes Made
- CREATED: [list of new GEMINI.md files]
- UPDATED: [list of modified GEMINI.md files]
- MIGRATED: [list of content moved from GEMINI.md to README.md]
- CREATED: [list of new README.md files]
- FLAGGED: [any issues requiring human decision]

### Verification
- Directories audited: [count]
- GEMINI.md coverage: [count]/[total] (100%)
- GEMINI.md format: [count] pure index / [count] needed migration
...
- **Stub directories** (contain only `.gitkeep` or no code files) - these do not
  require GEMINI.md until code is added

DO NOT index (skip these files in GEMINI.md):
...
- This skill is for GEMINI.md/README.md synchronization specifically
