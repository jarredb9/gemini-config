...
See `.gemini/conventions/diff-format.md` for diff specification.
...
Documentation-only milestones (ALL files are .md, .rst, .txt, or GEMINI.md):
...
- `path/to/GEMINI.md` (index updates -- in same directory as code)
...
Key deliverables:
- GEMINI.md: Pure navigation index (tabular format)
- README.md: Invisible knowledge (if IK section non-empty)

**Acceptance Criteria**:

- GEMINI.md is tabular index only (no prose sections)
