...
Start: <invoke working-dir=".gemini/skills/scripts" cmd="python3 -m """ + EXPLORE_MODULE_PATH + """ --step 1 --category $ref --mode $mode""" + scope_arg + """" />"""

    # Command template (also has $var placeholders)
    command = f'<invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {EXPLORE_MODULE_PATH} --step 1 --category $ref --mode $mode{scope_arg}" />'
...
DO NOT modify commands. DO NOT skip steps. DO NOT interpret.
  ...
  <if_custom>
    <invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {MODULE_PATH} --step 2 --mode custom{scope_arg}" />
  </if_custom>
  <if_not_custom>
    <invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {MODULE_PATH} --step 2 --n {n} --mode $MODE{scope_arg}" />
  </if_not_custom>
</invoke_after>"""
...
def format_step_3_verification(info: dict, scope: str | None = None, retry: int = 0) -> str:
    ...
    if retry < 1:
        ...
        invoke_after = f"""<invoke_after>
  <if_revise>
    <invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {MODULE_PATH} --step 3 --mode custom{scope_arg} --retry 1" />
  </if_revise>
  <if_pass>
    <invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {MODULE_PATH} --step 4 --mode custom{scope_arg}" />
  </if_pass>
</invoke_after>"""
    else:
...
def format_step_4_dispatch_custom(info: dict, scope: str | None = None) -> str:
    ...
    scope_display = scope or "entire codebase"
    scope_arg = f" --scope {shlex.quote(scope)}" if scope else ""
    invoke_cmd = f'<invoke working-dir=".gemini/skills/scripts" cmd="python3 -m {EXPLORE_MODULE_PATH} --step 1 --category $CATEGORY_REF --mode code{scope_arg}" />'
